...
from include.helper_functions.kaggle_airbnb import identify_columns_missing_comments, create_llm_request_batches, build_model_api_payload
...
class TestKaggleAirbnb(unittest.TestCase):
    ...
    def test_build_model_api_payload(self):
        ...
        result = build_model_api_payload('context', 'table', columns)
        self.assertEqual(result['context'], 'context')
        self.assertEqual(result['tables'][0]['name'], 'table')
        self.assertEqual(result['tables'][0]['columns'], columns)
